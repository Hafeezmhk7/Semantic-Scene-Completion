...
class AlignedShapeLatentPerceiver(ShapeAsLatentPerceiver):
    ...
    def __init__(self, *, device, dtype, num_latents, point_feats=0, embed_dim=0,
        ...
        print(f"  ── IDEAS ───────────────────────────────────────────────────────")
        print(f"  decoder_pos_enc:    {decoder_pos_enc}")
    ...
    def decode(self, latents, volume_queries=None, return_semantic_features=False,
        ...
        if self.token_cond_flag:
            ...
            if ('B' in self.token_cond_approach and
                ...
                W = F.softmax(self.token_cat_assign, dim=-1)
                pred_c = self.last_scene_layout_pred
    ...
    def forward(self, pc, feats, volume_queries, sample_posterior=True,
        ...
        # SceneLayoutHead before decode() so Idea 2B can use pred_centroids
        ...
        # NOTE: last_anchor_pred removed. Use last_predicted_anchors_from_tokens instead.
        # last_predicted_anchors_from_tokens is set inside decode() above.
