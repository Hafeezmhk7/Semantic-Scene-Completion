...
def compute_reconstruction_loss(prediction, target, batch_size, color_weight=1.0, log_scale_loss=False):
    
    eps = 1e-8
    
    # Always separate scale out when using log-scale loss
    if log_scale_loss:
        diff_pos      = prediction[:, :, 0:3]  - target[:, :, 0:3]
        diff_color    = prediction[:, :, 3:6]  - target[:, :, 3:6]
        diff_opacity  = prediction[:, :, 6:7]  - target[:, :, 6:7]
        # Log-space scale loss — symmetric penalty across magnitudes
        diff_scale    = torch.log(prediction[:, :, 7:10] + eps) - torch.log(target[:, :, 7:10] + eps)
        diff_rotation = prediction[:, :, 10:14] - target[:, :, 10:14]
        
        loss = (torch.norm(diff_pos, p=2)
              + torch.norm(diff_color, p=2) * color_weight
              + torch.norm(diff_opacity, p=2)
              + torch.norm(diff_scale, p=2)
              + torch.norm(diff_rotation, p=2))
        return loss / batch_size
...
parser.add_argument('--log_scale_loss', action='store_true', default=False,
                    help='Use log-space L2 loss for scale parameters (symmetric penalties)')
    ...
    with torch.no_grad():
        for i_batch, batch_data in enumerate(tqdm(dataloader, desc="Evaluating", leave=False)):
            ...
            batch_l2_error = compute_reconstruction_loss(
                ...
                log_scale_loss=args.log_scale_loss
            )
